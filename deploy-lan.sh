...
echo -e "  ${YELLOW}  只需在 App 内「系统设置 → 服务器地址」改一次即可，无需重新打包。${NC}"
...
echo -e "    3. 手机安装通用版 APK cotton-recognition.apk（无需按 IP 重新打包）"
echo -e "    4. 首次打开 App → 我的 → 系统设置 → 服务器地址，填 http://${LAN_IP}:${BACKEND_PORT}"
echo -e "       （或点「自动搜索服务器」，无需知道 IP）"
echo ""
...
echo -e "    1. 手机安装通用版 APK（cotton-recognition.apk，无需按 IP 重新打包）"
echo -e "    2. 在 App 内填写或自动搜索服务器地址: http://${LAN_IP}:${BACKEND_PORT}"
echo -e "    3. 用管理员账号登录后即可开始识别（账号见交付清单）"
echo ""