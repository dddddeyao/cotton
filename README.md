...
默认部署后暴露后端 API。Android 端**首次打开会自动搜索局域网内的后端**（找到即保存并进入主界面），失败时会按 8/15/30/60 秒自动重试；也可在 **我的 → 系统设置 → 服务器地址** 手动填写或点「自动搜索服务器」。地址格式：
...
- 服务器地址：**首次启动自动搜索局域网后端**（失败按 8/15/30/60 秒自动重试），也可在 App 内手动配置（含连接测试与自动搜索），换电脑或换 IP 都无需重新打包
