...
3. 打开 App → **自动搜索服务器**（几秒）→ 找到后直接进入，**不用填 IP**
   - 没找到会显示「未找到服务器」，并按 **8/15/30/60 秒自动重试**，也可点「**重新搜索**」
   - 需要手动指定时：点「**手动填写地址**」，或走 **我的 → 系统设置 → 服务器地址** → 填 `http://刚才记下的IP:8088` → 「**保存并测试连接**」
4. 用账号登录 → 拍照识别，正常出结果就完成了
