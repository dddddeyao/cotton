...
> 状态：**已重新启用**（2026-09-20，随 v1.1.0 交付）。本文档既是方案设计说明，也是该功能的实现说明。
>
> 相关提交：`8696093`（首次实现）→ `7a7df7a`（回滚）→ 本次以 `git cherry-pick -n 8696093` 恢复并复核
> 如需在其它分支重复恢复：`git cherry-pick 8696093`
> 交付形态：`cotton-recognition.apk`（通用版，地址留空，由 App 内配置）
...
- 当时主流程（部署 + 识别）刚跑通，交接在即，优先保证**稳定性**，不在交付前引入原生依赖与新交互。
- 当时的替代做法：现场用 `.\build-apk-lan.ps1 -LanIP <现场IP>` 重新打包，或确认现场网段与预置 IP 一致。

> ✅ **2026-09-20 更新**：交付场景最终确认为「部署一次、长期不再改动，且现场不一定具备开发环境」，
> 原来的替代做法（现场重新打包）不可行，因此本功能已重新启用并随 v1.1.0 交付。
